...
function setArray(key,array){
    const newCouunter = JSON.stringify(array);
    localStorage.setItem(key, newCouunter); 
}
function getArray(){
    const objectLocalStorage = localStorage.getItem("counter");
    if(objectLocalStorage){
        const objectItem =JSON.parse(objectLocalStorage);
        return objectItem;
    } }
    function getArray1(){
        const objectLocalStorage1 = localStorage.getItem("counter1");
        if(objectLocalStorage1){
            const objectItem1 =JSON.parse(objectLocalStorage1);
            return objectItem1;
        } 
    }
  
btnResult.onclick = () =>{
    let first = getArray("counter");
    let second = getArray1("counter1");
   checkResult(first, second); 
}
function checkResult(old,neww){
        if(old>neww){
            alert("Erste mall war besser")
        }else if(old<neww){
            alert("Zweite mall war besser")
        }else{
            alert("Gleich")
        }
    }
